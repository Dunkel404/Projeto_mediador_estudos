...
export const DualSandbox: React.FC<DualSandboxProps> = ({
  ...
  return (
    <div className={`flex flex-col h-full bg-[#12141a] border border-[#242933] ${className}`}>
          ...
          {activeTab === 'shader' && (
            <>
              <button
                onClick={handleReset}
                title="Reset Code"
                className="p-1.5 text-slate-400 hover:text-white border border-[#242933] hover:bg-[#181b22] transition-colors"
              >
                <RotateCcw className="w-3.5 h-3.5" />
              </button>
              <button
                onClick={handleRun}
                className="flex items-center gap-1 px-3 py-1 bg-[#00f0ff] hover:bg-[#38bdf8] text-black font-mono font-bold text-xs transition-colors"
              >
      ...
      {/* Main Sandbox Area: Split View on Large Screens */}
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 min-h-0">
        ...
        <div className="relative w-full h-[320px] lg:h-full border-b lg:border-b-0 lg:border-r border-[#242933]">
          {activeTab === 'shader' ? (
            ...
            <ParametricScene3D functionType="saddle" />
          )}
        </div>
